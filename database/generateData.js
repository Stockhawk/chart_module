...
// var companyData = createCompanies(5);
  ...
  (function loop(base, i){
    for (let k = 0; k < possible.length; k++) {
      if (i > 1) loop( base + possible[k], i-1);
        ...
        allTickers.push({
          id: allTickers.length + 1,
          ticker: base + possible[k],
          company: base + possible[k]});
      } 
    }
  })("", digits);
  return allTickers;
...
var companyData2 = generateTickers(5);

module.exports.generateData = (index) => {
  return {
    id: companyData2[index].id,
    stockId: companyData2[index].ticker,
    stockInfo: {
      stockCompany: companyData2[index].company,
